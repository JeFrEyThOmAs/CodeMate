...
export async function getMyRecentSessions(req , res){
    try{
        ...
        // get the status where the host is either host or participant
...
export async function joinSession(req , res){
    try{
        ...
        // check if the session is already full , ie s a participant
...
export async function endSession(req , res){
    try{
        ...
        if(session.host.toString() !== userI.toString()){
            return res.status(401).json({message : "only the host can end the session"})
        }
